...
def band_structure(vecs, N=1, n_m=3):
    ...
    comp = jnp.argmax(
        jnp.stack([jnp.abs(vecs[i :: n_m * N, :]).sum(axis=0) for i in range(n_m * N)]),
        axis=0,
    )
...
def dos(w, wm, gm):
    ...
    return (
        1
        / (wm.size * jnp.pi)
        * jnp.sum(gm / ((jnp.expand_dims(w, 1) - wm) ** 2 + gm**2), axis=1)
    )
...
def analytic_chain(l: float, kzs):
    ...
    def inner(kz: float) -> tuple[float, float, float, float]:
        ...
        gamma_parallel = (
            (k_null_dimless**2 - kz**2) / k_null_dimless**2
            if k_null_dimless > kz
            else 0
        )
        gamma_perp = (
            1 / 2 * (k_null_dimless**2 + kz**2) / k_null_dimless**2
            if k_null_dimless > kz
            else 0
        )
        ...
        return [
            scale_d * delta_perp,
            scale_d * delta_parallel,
            scale_g * gamma_perp,
            scale_g * gamma_parallel,
        ]

    k_null_dimless, scale_d, scale_g = (
        K_0 * l,
        SCALE / (4 * jnp.pi * l),
        -SCALE / (4 * l),
    )
...
def analytic_flat_bands(a: int, b: int, l: float, o: float):
    ...
    r = jnp.min(
        jnp.linalg.norm(
            pos[pos[:, 1] != 0] - jnp.expand_dims(pos[pos[:, 1] == 0], 1), axis=2
        ),
        axis=1,
    )
    ...
    E1 = (
        -SCALE
        / (8 * jnp.pi * r)
        * (jnp.cos(kr) - jnp.cos(kr) / kr**2 - jnp.sin(kr) / kr)
    )
...
def to_tikz(a, b, l, o, N: int = 2):
    ...
    cc = [
        (
            "\\vb{p}_{" + f"{i % (a+b)}{int(i/(a+b))}" + "}",
            tikz_colors[(i % (a + b)) % len(tikz_colors)],
            pos[:2],
        )
        for i, pos in enumerate(twisted_chains(a, b, l, o, N))
    ]
    ...
    draw_l2 = [
        to_tikz_id(x) for x in map(lambda x: x[0], filter(lambda x: x[2][1] > 0, cc))
    ]
    draw_l1 = [
        to_tikz_id(x) for x in map(lambda x: x[0], filter(lambda x: x[2][1] == 0, cc))
    ]
    footer = (
        ...
        + """) -- ("""
        + f"{draw_l2[1]}"
        + """)node[midway, above]{$\Lambda_2$};
    \\draw ("""
        + f"{draw_l1[0]}"
        + """) -- ("""
        + f"{draw_l1[1]}"
        + """)node[midway, above]{$\Lambda_1$};
    \\draw ("""
        + f"{draw_l1[a]}"
        + """) -- ("""
        + f"{draw_l2[b]}"
...
if __name__ == "__main__":
    ...
    ax1.legend(
        ax1.lines, (r"$D_{\perp}(k)$", r"$D_{\parallel}$(k)", "finite", LL_LABEL)
    )
    ...
    ax2.legend(
        ax2.lines, (r"$G_{\perp}(k)$", r"$G_{\parallel}(k)$", "finite", LL_LABEL)
    )
    ...
    fig = plt.figure()
    canvas = gridspec.GridSpec(2, 1, wspace=0, hspace=0.05, height_ratios=[0.05, 1])
    inlet = gridspec.GridSpecFromSubplotSpec(
        1, 2, subplot_spec=canvas[1], wspace=0, hspace=0
    )
    ...
    cb = Colorbar(
        ax=cbax, mappable=sc, orientation="horizontal", ticklocation="top", label=r"$G$"
    )
    plt.show()
    plt.close()
    # plt.savefig('dos.pdf')
    ...
    lim1, lim2 = (
        max([jnp.max(arr) for arr in flat_bands]) + 2,
        min(jnp.min(arr) for arr in flat_bands) - 2,
    )
    ...
    flat_bands = [analytic_flat_bands(a, b, lc, o) for a, b in vals]
    fig = plt.figure()
    canvas = gridspec.GridSpec(2, 1, wspace=0, hspace=0.17, height_ratios=[0.05, 1])
    inlet = gridspec.GridSpecFromSubplotSpec(
        1, 3, subplot_spec=canvas[1], wspace=0, hspace=0
    )
    ...
    canvas = gridspec.GridSpec(2, 1, wspace=0, hspace=0.17, height_ratios=[0.05, 1])
    inlet = gridspec.GridSpecFromSubplotSpec(
        1, 3, subplot_spec=canvas[1], wspace=0.22, hspace=0
    )
    inlet = [plt.subplot(i) for i in inlet]
    for i, ax in enumerate(inlet):
        if i:
        ...
        shift, linewidth, vecs = spectrum(
            disorder(twisted_chains(a, b, lc, o, 30), disorder_params[i] * o)
        )
        ks = band_structure(vecs, a + b)
        idxs = jnp.argwhere(linewidth <= 0.1)[:, 0]
        sc = ax.scatter(ks[idxs], shift[idxs], c=linewidth[idxs])
    ...
    for i, ax in enumerate(inlet):
        ...
        shift, linewidth, vecs = spectrum(
            twisted_chains(a, b, lc, os[i] * LAMBDA_0, 30)
        )
        ks = band_structure(vecs, a + b)
        idxs = jnp.argwhere(linewidth <= 0.1)[:, 0]
        sc = ax.scatter(ks[idxs], shift[idxs], c=linewidth[idxs])
